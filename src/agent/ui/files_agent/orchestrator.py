...
search_file_all_drives(query, extensions=None, limit=20) – Search ALL drives (C:, D:, …) and the full home directory tree for a file by name/pattern. Use when the user asks to "find" or "search" for ANY file on the laptop — not just in Downloads. Returns file_path set to the first match for instant delivery.
...
def _build_skill_context() -> str:
    ...
    return (
        ...
        "- When the user mentions a folder you cannot find under Home/Downloads/Desktop/Documents, use list_directory('C:\\\\') to check the drive root, or use search_file_all_drives(query) to locate it anywhere on the laptop.\n"
