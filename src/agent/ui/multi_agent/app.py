...
def _render_pa_chat(pa: dict) -> None:
    ...
    if st.session_state[mk("processing")] and st.session_state[mk("command")]:
        ...
        st.session_state[mk("messages")].append({"role": "user", "content": command})
        ...
        if agents_needed is not None and pa_skills:
            ...
            agents_needed = filtered if filtered else agents_needed
        ...
        with st.chat_message("assistant"):
            ...
            if len(agents_needed) == 1:
                ...
                st.markdown(f"{icon} **Routing to {label}…**")
            ...
            with st.status("⚡ Executing workflow…", expanded=True) as status_box:
                try:
                    ...
                    label_map = {"success": ("✅ Workflow complete", "complete"),
                                 "partial": ("⚠️ Workflow partial", "error")}
                    lbl, st_state = label_map.get(wf_status, ("❌ Workflow failed", "error"))
