...
"""
    ...
    def reason_and_act(
        ...
        system_prompt = (
            ...
            "- If a tool observation starts with 'Error:', you MUST report the "
            "failure honestly in final_answer. NEVER claim success when the "
            "observation shows an error. Do not fabricate or assume the operation "
            "succeeded — tell the user exactly what went wrong.\n"
            "- If a tool returns an error, try a different approach only if there "
            "is a clearly better alternative (e.g. try a different parameter). "
            "Otherwise explain the issue directly.\n"
