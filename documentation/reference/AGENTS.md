...
| **Monthly view** | "What do I have this month?" � "How many calendar events do I have in March?" |
...
> **Different from Files Agent:** The Files Agent performs immediate actions. The File Organizer Agent uses a **conversational approval workflow** � scan ? propose ? preview ? approve ? apply. It also manages archival policies and maintains Octa Bot's own runtime area under `your_data/`.
...
- **Plans are persisted** to `your_data/organizer_pending_plans.json` � survive session restarts
...
`your_data/habits.json` � flat list of habit objects:
```json
{
  "id": "a1b2c3d4",
  "name": "Morning Run",
...
`your_data/habit_logs.json` � flat list of daily log entries:
```json
{
  "habit_id": "a1b2c3d4",
...
| **Full report** | "Analyse Intellect Design Arena" � "Generate a stock report for Nvidia" |
